    ...
    async ({ params, body, staffCtx }) => {
      ...
      try {
        const updated = await prisma.$transaction(async (tx) => {
          ...
          if (
            body.status !== undefined &&
            (await applyStatusChange(tx, order, body.status, staffCtx, staffCtx.staff.username))
          ) {
          ...
          const saved = await tx.order.update({
            where: statusGuard(order.id, from),
            data,
            include: { items: true },
          });

          if (body.status !== undefined) {
            await ensureDeliveryWarranties(tx, saved, body.status, staffCtx);
          }
