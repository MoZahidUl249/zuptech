...
function Row({
  ...
  return (
    <div
      className={
        strong
          ? "flex items-baseline justify-between border-t-2 border-zup-body/20 pt-2.5 text-ui-base font-extrabold"
          : "flex items-baseline justify-between text-ui-sm"
      }
    >
  ...
  return (
    <div className="print-doc rounded-2xl border border-zup-body/8 bg-white px-7 py-7 text-zup-body sm:px-9 sm:py-9">
      ...
      <div className="flex flex-wrap items-start justify-between gap-5 border-b-2 border-zup-body/12 pb-5">
        <div>
          <p className="text-ui-lg font-extrabold tracking-[-0.02em]">ZUP TECH</p>
          <p className="mt-1 max-w-[240px] text-ui-micro leading-relaxed text-zup-gray">
            {contact.street}
        ...
        <div className="text-right">
          <p className="text-ui-micro font-bold uppercase tracking-[0.18em] text-zup-gray">
            {isVoid ? "Invoice (void)" : "Invoice"}
          </p>
          <p className="mt-1 text-ui-xl font-extrabold tracking-[-0.02em]">{invoice.id}</p>
          <p className="mt-1 text-ui-micro text-zup-gray">
            Order {invoice.orderId} · {fmtDate(invoice.createdAt)}
          </p>
          {isVoid ? (
            <p className="mt-1.5 inline-block rounded border-2 border-destructive px-2 py-0.5 text-ui-micro font-extrabold uppercase tracking-[0.12em] text-destructive">
              Void
            </p>
          ) : null}
        </div>
      ...
      <div className="mt-5 grid gap-5 sm:grid-cols-[1fr_auto]">
        <div>
          ...
          <p className="mt-1.5 text-ui-base font-bold">{invoice.customer}</p>
          <p className="text-ui-xs text-zup-mid">{invoice.phone}</p>
          <p className="mt-0.5 max-w-[320px] text-ui-xs leading-relaxed text-zup-gray">
            {invoice.address}
          </p>
        </div>
        <div className="flex flex-col gap-1 text-ui-xs sm:text-right">
          <p>
      ...
      <table className="mt-6 w-full border-collapse text-left">
        <thead>
          <tr className="border-b-2 border-zup-body/12">
            <th className="py-2 text-ui-micro font-bold uppercase tracking-[0.12em] text-zup-gray">
              Item
            </th>
            <th className="py-2 text-ui-micro font-bold uppercase tracking-[0.12em] text-zup-gray">
              SKU
            </th>
            <th className="py-2 text-right text-ui-micro font-bold uppercase tracking-[0.12em] text-zup-gray">
              Qty
            </th>
            <th className="py-2 text-right text-ui-micro font-bold uppercase tracking-[0.12em] text-zup-gray">
              Unit price
        ...
        <tbody>
          {invoice.items.map((item) => (
            <tr key={item.productId} className="border-b border-zup-body/8">
              <td className="py-2.5 text-ui-sm font-semibold">{item.name}</td>
              <td className="py-2.5 text-ui-xs text-zup-gray">{item.sku}</td>
              <td className="py-2.5 text-right text-ui-sm [font-variant-numeric:tabular-nums]">
                {item.qty}
              </td>
              <td className="py-2.5 text-right text-ui-sm [font-variant-numeric:tabular-nums]">
                {taka(item.unitPrice)}
              </td>
              <td className="py-2.5 text-right text-ui-sm font-bold [font-variant-numeric:tabular-nums]">
                {taka(item.lineTotal)}
              </td>
            </tr>
          ))}
      ...
      {invoice.notes ? (
        <p className="mt-6 max-w-[420px] whitespace-pre-wrap text-ui-xs leading-relaxed text-zup-gray">
          {invoice.notes}
        </p>
      ) : null}
      ...
      <div className="mt-10 grid gap-8 sm:grid-cols-2">
        <div>
          ...
          <p className="mt-1.5 text-ui-micro text-zup-gray">
            Prepared by
            {invoice.preparedBy ? (
      ...
      <p className="mt-7 text-center text-ui-micro text-zup-faint">
