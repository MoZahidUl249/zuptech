 ...
 */
export function useQuote(items: QuoteItem[], insideDhaka?: boolean) {
  // Serialize so the effect re-runs on content change, not array identity.
  const key = JSON.stringify({ items, insideDhaka });
  const [result, setResult] = useState<QuoteResult | null>(null);

  useEffect(() => {
    const { items, insideDhaka } = JSON.parse(key) as {
      items: QuoteItem[];
      insideDhaka?: boolean;
    };
    ...
    fetch("/api/pricing/quote", {
      ...
      body: JSON.stringify({ items, insideDhaka }),
      signal: ctrl.signal,
    })
